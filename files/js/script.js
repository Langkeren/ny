...
let lastEffect;
const EFFECTS = [crackleEffect, fallingLeavesEffect, floralEffect, crossetteEffect]

function randomEffectSimple() {
    return EFFECTS[Math.random() * EFFECTS.length | 0];
}

function randomEffect(){
    let effect = randomEffectSimple();
    while (effect === lastEffect) {
        effect = randomEffectSimple();
    }
    lastEffect = effect;
    return effect;
}
...
function randomOffset(num, bound = 60, basic = 30){
    const offset =  basic + bound * Math.random()
    return Math.random() < 0.5 ? num -  offset : num + offset
}
...
class Shell {
    ...
    launch(position, launchHeight) {
        ...
        if (userClick && (hasText || hasImg)){
            ...
            if (userClickCount === 2 && hasText){
                onDeath = textEffect
            }else if (userClickCount < 5){
                onDeath = myEffect;
...
const Star = {
    ...
    addTextStart(x, y, color, prevX, prevY, speed, life, speedOffX, speedOffY) {
        const instance = this._pool.pop() || this._new();
        ...
        instance.x = x;
        instance.y = y;
        instance.prevX = prevX;
        instance.prevY = prevY;
